...
const ShopEditPage = () => {
    ...
    const fetchShopDetails = async () => {
      try {
        ...
        if (response.data && response.data.code === 200) {
          ...
          if (userShopResponse.data && userShopResponse.data.code === 200 && 
              userShopResponse.data.data && userShopResponse.data.data.id === parseInt(shopId)) {
            setIsOwner(true);
            ...
            if (shopData.shopLogo) {
              setPreviewLogo(shopData.shopLogo);
            }
          } else {
            // 不是店铺所有者，返回店铺详情页
        ...
        setErrors({ general: '获取店铺信息失败，请稍后重试' });
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="shop-edit-page pb-16">
      {/* 顶部导航栏 */}
      <div className="fixed top-0 left-0 right-0 bg-primary text-white z-10 max-w-[375px] mx-auto">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-medium">店铺设置</h1>
