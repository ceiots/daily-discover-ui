        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white rounded-lg overflow-hidden shadow-sm mb-6">
              <div className="p-3">
                <h3 className="text-sm font-medium mb-2">退款进度</h3>
                <div className="space-y-4 mt-4">
                  <div className="flex">
                    <div className="timeline-dot bg-green-500"></div>
                    <div className="ml-3">
                      <div className="text-xs font-medium">申请退款</div>
                      <div className="text-xs text-gray-500">
                        {new Date(refundData.createdAt).toLocaleString('zh-CN')}
                      </div>
                    </div>
                  </div>
                  ...
                  {refundData.status >= 1 && (
                    <div className="flex">
                      <div className={`timeline-dot ${refundData.status === 2 ? 'bg-red-500' : 'bg-green-500'}`}></div>
                      <div className="ml-3">
                        <div className="text-xs font-medium">
                          {refundData.status === 2 ? '商家拒绝' : '商家同意'}
                        </div>
                        <div className="text-xs text-gray-500">
                          {refundData.updatedAt ? new Date(refundData.updatedAt).toLocaleString('zh-CN') : '-'}
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {refundData.status === 3 && (
                    <div className="flex">
                      <div className="timeline-dot bg-green-500"></div>
                      <div className="ml-3">
                        <div className="text-xs font-medium">退款完成</div>
                        <div className="text-xs text-gray-500">
                          {refundData.completedAt ? new Date(refundData.completedAt).toLocaleString('zh-CN') : '-'}
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {refundData.status === 4 && (
                    <div className="flex">
                      <div className="timeline-dot bg-gray-500"></div>
                      <div className="ml-3">
                        <div className="text-xs font-medium">退款取消</div>
                        <div className="text-xs text-gray-500">
                          {refundData.canceledAt ? new Date(refundData.canceledAt).toLocaleString('zh-CN') : '-'}
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {refundData.status === 0 && (
              <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-3">
                <button
                  ...
                  disabled={loading}
                  className="w-full py-2.5 rounded-full bg-gray-200 text-gray-700 text-sm font-medium"
                >
                  取消申请
                </button>
              </div>
          ...
          <div className="text-center py-8 text-gray-500">未找到退款记录</div>
        )}
      </div>
    </div>
  );
